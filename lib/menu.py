...
class Menu (object):
    ...
    def __init__(self, window, font, title, **kwargs):
        ...
        # Transparencia
        if kwargs.get("bgalpha") is not None: bgalpha = kwargs.get("bgalpha")
        else: bgalpha = MENU_ALPHA
        # Color de fondo a dibujar
        if kwargs.get("bgcolor") is not None: bgcolor = kwargs.get("bgcolor")
        else: bgcolor = MENU_BGCOLOR
        self.bgColor = (bgcolor[0], bgcolor[1], bgcolor[2], int(255 * (1 - (100 - bgalpha) / 100.0)))
        # Color del titulo
        if kwargs.get("bgcolor_title") is not None: bgcolor = kwargs.get("bgcolor_title")
        else: bgcolor = MENU_TITLE_BG_COLOR
        self.bgColorTitle = (bgcolor[0], bgcolor[1], bgcolor[2], int(255 * (1 - (100 - bgalpha) / 100.0)))
        # Menu centrado
        if kwargs.get("centered") is not None: self.centered_option = kwargs.get("centered")
        else: self.centered_option = MENU_CENTERED_TEXT
        # Define si se escribe el rectangulo en el indice seleccionado o no
        if kwargs.get("draw_selected") is not None: self.drawselrect = kwargs.get("draw_selected")
        else: self.drawselrect = MENU_SELECTED_DRAW
        # Tamaño de la fuente
        if kwargs.get("font_size") is not None: self.fontsize = kwargs.get("font_size")
        else: self.fontsize = MENU_FONT_SIZE
        # Tamaño de la fuente del titulo
        if kwargs.get("font_size_title") is not None: self.fontsize_title = kwargs.get("font_size_title")
        else: self.fontsize_title = MENU_FONT_SIZE_TITLE
        # Fuente del menú
        self.font = pygame.font.Font(font, self.fontsize)
        self.fonttitle = pygame.font.Font(font, self.fontsize_title)
        # Color de la fuente
        if kwargs.get("font_color") is not None: self.fontColor = kwargs.get("font_color")
        else: self.fontColor = MENU_FONT_COLOR
        # Alto del menu en pixeles
        if kwargs.get("height") is not None: self.height = kwargs.get("height")
        else: self.height = MENU_HEIGHT
        # Distancia entre opciones
        if kwargs.get("option_margin") is not None: self.optiondy = kwargs.get("option_margin")
        else: self.optiondy = MENU_OPTION_MARGIN
        # Ancho del rectangulo dibujado
        if kwargs.get("rect_width") is not None: self.rectwidth = kwargs.get("rect_width")
        else: self.rectwidth = MENU_SELECTED_WIDTH
        # Color del item seleccionado
        if kwargs.get("color_selected") is not None: self.selectedcolor = kwargs.get("color_selected")
        else: self.selectedcolor = MENU_SELECTEDCOLOR
        # Sombra de las opciones
        if kwargs.get("option_shadow") is not None: self.option_shadow = kwargs.get("option_shadow")
        else: self.option_shadow = MENU_OPTION_SHADOW
        # ancho del menu en pixeles
        if kwargs.get("width") is not None: self.width = kwargs.get("width")
        else: self.width = MENU_WIDTH
        ...
        self.bgRect = [(self.posx, self.posy), (self.posx + self.width, self.posy), (self.posx + self.width, self.posy + self.height), (self.posx, self.posy + self.height)]
        # Punto a dibujar de las opciones
        if kwargs.get("draw_region_x") is not None: self.drawRegionX = kwargs.get("draw_region_x")
        else: self.drawRegionX = MENU_DRAW_X
        if kwargs.get("draw_region_y") is not None: self.drawRegionY = kwargs.get("draw_region_y")
        else: self.drawRegionY = MENU_DRAW_Y
        ...
        self.titleRect = [(self.posx, self.posy), (self.posx + self.width, self.posy), (self.posx + self.width, self.posy + self.fontsize_title / 2), \
                          (self.posx + title_width + 25, self.posy + self.fontsize_title / 2), (self.posx + title_width + 5, self.posy + self.fontsize_title + 5), \
    ...
    # Agregar un select al menu
    def addSelector(self, title, values, event, *args, **kwargs):
        if kwargs.get("index") is not None: self.actual.opciones.insert(kwargs.get("index"), [SELECTOR, Selector(title, values, event, *args)])
        else: self.actual.opciones.append([SELECTOR, Selector(title, values, event, *args)])
        self.actual.size += 1
        if self.actual.size > 1: self.actual.posOptionY += -self.actual.fontsize / 2 - self.actual.optiondy / 2
        ...
        for option in self.actual.opciones:
            ...
            if self.actual.drawselrect and (dy == self.actual.index):
                ...
                pygame.draw.line(surface, self.actual.selectedcolor, (self.actual.posOptionX + text_dx - 10, self.actual.posOptionY + dy * (self.actual.fontsize + self.actual.optiondy) - text_dy + 2), \
                                 (self.actual.posOptionX - text_dx_tl + 10, self.actual.posOptionY + dy * (self.actual.fontsize + self.actual.optiondy) - text_dy + 2), self.actual.rectwidth)
                pygame.draw.line(surface, self.actual.selectedcolor, (self.actual.posOptionX + text_dx - 10, self.actual.posOptionY + dy * (self.actual.fontsize + self.actual.optiondy) + text_dy - 2), \
                                (self.actual.posOptionX + text_dx - 10, self.actual.posOptionY + dy * (self.actual.fontsize + self.optiondy) - text_dy + 2), self.actual.rectwidth)
    ...
    # Mueve el selector (si existe) a la izquierda
    def left(self):
    ...
    def select(self):
        try:
            ...
            # Si el tipo es una función
                ...
                if len(self.actual.opciones[self.actual.index][2]) > 0:opcion(*self.actual.opciones[self.actual.index][2])
                else: opcion()
            # Si el tipo es Null
