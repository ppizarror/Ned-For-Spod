...
class Window:
    ...
    def getDisplayList(self):
        ...
        for resolution in SCREEN16_9:
            if resolution[0] <= max_w and resolution[1] <= max_h:
                disponible.append((FORMAT_16_9.format(resolution[0], resolution[1]),
                                   FORMAT_VALUE.format(resolution[0], resolution[1])))
        for resolution in SCREEN4_3:
            if resolution[0] <= max_w and resolution[1] <= max_h:
                disponible.append((FORMAT_4_3.format(resolution[0], resolution[1]),
                                   FORMAT_VALUE.format(resolution[0], resolution[1])))
        while True:
    ...
    def setFullscreen(self):
        ...
        os.environ['SDL_VIDEO_CENTERED'] = '1'
        self.display = pygame.display.set_mode((self.width, self.height),
                                               FULLSCREEN | pygame.HWSURFACE | pygame.DOUBLEBUF | pygame.NOFRAME, 32)
