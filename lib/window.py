...
class Window:
    ...
    def __init__(self, configs, title, icon, info, **kwargs):
        ...
        # Se crea la pantalla
        if configs.isTrue("WINDOWED"): self.display = pygame.display.set_mode((self.width, self.height), pygame.DOUBLEBUF | pygame.HWSURFACE, 32)
        else: self.display = pygame.display.set_mode((self.width, self.height), FULLSCREEN | pygame.HWSURFACE | pygame.DOUBLEBUF | pygame.NOFRAME, 32)
        pygame.display.set_icon(icon)
        pygame.display.set_caption(title)
        # Si se desabilita el raton
        if not configs.isTrue("SHOW_MOUSE"): pygame.mouse.set_visible(False)
    ...
    def getDisplayList(self):
        actual_value = FORMAT_VALUE.format(self.config.getValue("WIDTH"), self.config.getValue("HEIGHT"))
        ...
        for resolution in SCREEN16_9:
            if resolution[0] <= max_w and resolution[1] <= max_h:
                disponible.append((FORMAT_16_9.format(resolution[0], resolution[1]), FORMAT_VALUE.format(resolution[0], resolution[1])))
        for resolution in SCREEN4_3:
            if resolution[0] <= max_w and resolution[1] <= max_h:
                disponible.append((FORMAT_4_3.format(resolution[0], resolution[1]), FORMAT_VALUE.format(resolution[0], resolution[1])))
        while True:
            if disponible[0][1] == actual_value: break
            else:
    ...
    def setFullscreen(self):
        os.environ['SDL_VIDEO_CENTERED'] = '1'
        self.display = pygame.display.set_mode((self.width, self.height), FULLSCREEN | pygame.HWSURFACE | pygame.DOUBLEBUF | pygame.NOFRAME, 32)

    # Cambia a pantalla reducida
    def setWindowed(self):
        self.display = pygame.display.set_mode((self.width, self.height), pygame.DOUBLEBUF | pygame.HWSURFACE, 32)
    ...
    def update(self):
        ...
        if self.config.isTrue("WINDOWED"): self.display = pygame.display.set_mode((self.width, self.height), pygame.DOUBLEBUF | pygame.HWSURFACE, 32)
        else: self.display = pygame.display.set_mode((self.width, self.height), FULLSCREEN | pygame.HWSURFACE | pygame.DOUBLEBUF | pygame.NOFRAME, 32)
