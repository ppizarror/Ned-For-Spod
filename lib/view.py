...
class View:
    ...
    def __init__(self, window, clock, world, lang, viewConfig, menus):
        ...
        :param viewConfig: Configuraciones de la vista
        ...
        self.posOutOfRoad = (
            self.windowWidth * 0.5 - 100, self.windowHeight * 0.95)  # posición del mensaje -fuera de la pista-
        self.resultsButtonNextPos = (int((self.windowWidth + 680) / 2) - 15, int(
            (self.windowHeight + 400) / 2) - 5)  # posicion del boton siguiente del plano de resultados
        self.resultsButtonQuitPos = (int((self.windowWidth - 680) / 2) + 15, int(
        ...
        self.resultsScreenPos = (int((self.windowWidth - 680) / 2),
                                 int((self.windowHeight - 400) / 2) - 25)  # posición en pantalla de los resultados
        self.revolPos = (self.windowWidth - 20, self.windowHeight - 60)  # posición del cuenta-revoluciones
        self.revolRect = pygame.image.load(
            getImages("revol_shadowed")).convert_alpha()  # textura del cuenta-revoluciones
        ...
        self.revolGraph = revolGraph(33, (self.revolPos[0] - 170, self.revolPos[1] + 7), 100,
        ...
        if state == STATE_PLAY and self.isPlaying:
            ...
            while True:
                ...
                while True:
                    ...
                    if -bgSize[0] < drawx < self.windowWidth and -bgSize[
            ...
            if self.show_mark_ground:
                for marca_tierra in self.map.getMarcasTierra():
                    ...
                    if (0 <= drawxini <= self.windowWidth and 0 <= drawyini <= self.windowHeight) or \
                        ...
                        pygame.draw.line(self.screen, marca_tierra[2], (drawxini, drawyini), (drawxfin, drawyfin),
            ...
            if self.show_mark_track:
                for marca_camino in self.map.getMarcasFrenado():
                    ...
                    if (0 <= drawxini <= self.windowWidth and 0 <= drawyini <= self.windowHeight) or \
                            (0 <= drawxfin <= self.windowWidth and 0 <= drawyfin <= self.windowHeight):
                        pygame.draw.line(self.screen, marca_camino[2], (drawxini, drawyini), (drawxfin, drawyfin),
            ...
            if not self.map.getPlayer().finishedLap():
                ...
                if self.show_player:
                    ...
                    message = self.map.getPlayer().draw(self.screen, time, True, self.window,
                                                        draw_ruedas=self.show_ruedas, show_ghost=self.show_ghost)
                else:
                    message = STATE_NULL
                ...
                if self.show_ui:
                    ...
                    self.screen.blit(self.speedFont.render(str(self.map.getPlayer().getVelKph()), 1, COLOR_VELOCIMETER),
                                     (self.speedPos[0] - 10, self.speedPos[1] + 10))
                    self.screen.blit(self.speedFont.render(METRICS, 1, COLOR_VELOCIMETER),
                    ...
                    if cambio == 0:
                        ...
                        cambio = str(cambio)
                    self.screen.blit(self.speedFont.render(cambio, 1, COLOR_VELOCIMETER),
                    ...
                    self.screen.blit(self.lapFont.render(self.lang.get(11), 1, COLOR_VELOCIMETER),
                                     (self.lapPos[0] - 115, self.lapPos[1]))
                    self.screen.blit(
                        self.lapFontL.render(str(self.map.player.getLapPos()) + "/" + str(self.map.getLaps()), 1, \
                                             COLOR_VELOCIMETER), (self.lapPos[0] - 12, self.lapPos[1]))
                    # Dibuja el tiempo de vuelta
                    self.screen.blit(
                        self.lapTimeFont.render(self.lang.get(12) + " " * 6 + self.map.getPlayer().getLapTime(), 1,
                                                COLOR_VELOCIMETER), \
                    ...
                    if self.map.player.getLapPos() == 1:
                        for obj in self.map.getPlayer().getTrackObjetives():
                            if k == 0:
                                self.screen.blit(
                                    self.lapTimeFont.render(self.lang.get(19, self.map.getPlayer().getLapTime(obj)), 1,
                                                            COLOR_ORO), (20, 20 + 29 * k))
                                if self.map.getPlayer().getLapTimeNoFormat() > obj:
                                    pygame.draw.line(self.screen, COLOR_ALERT, (17, 31 + 29 * k), (155, 31 + 29 * k), 3)
                            if k == 1:
                                self.screen.blit(
                                    self.lapTimeFont.render(self.lang.get(20, self.map.getPlayer().getLapTime(obj)), 1,
                                                            COLOR_PLATA), (20, 20 + 29 * k))
                                if self.map.getPlayer().getLapTimeNoFormat() > obj:
                                    pygame.draw.line(self.screen, COLOR_ALERT, (17, 31 + 29 * k), (175, 31 + 29 * k), 3)
                            if k == 2:
                                self.screen.blit(
                                    self.lapTimeFont.render(self.lang.get(21, self.map.getPlayer().getLapTime(obj)), 1,
                                                            COLOR_BRONCE), (20, 20 + 29 * k))
                                if self.map.getPlayer().getLapTimeNoFormat() > obj:
                        ...
                        for obj in self.map.getPlayer().getTrackObjetives():
                            if k == 0:
                                if self.map.getPlayer().getFastLap()[1] < obj:
                                    self.screen.blit(self.lapTimeFont.render(self.lang.get(19, self.lang.get(46)), 1,
                                                                             COLOR_RESULT_LINE), (20, 20 + 29 * k))
                                else:
                                    self.screen.blit(
                                        self.lapTimeFont.render(self.lang.get(19, self.map.getPlayer().getLapTime(obj)),
                                                                1, COLOR_ORO), (20, 20 + 29 * k))
                                    if self.map.getPlayer().getLapTimeNoFormat() > obj:
                                        pygame.draw.line(self.screen, COLOR_ALERT, (17, 31 + 29 * k),
                                                         (155, 31 + 29 * k), 3)
                            if k == 1:
                                if self.map.getPlayer().getFastLap()[1] < obj:
                                    self.screen.blit(self.lapTimeFont.render(self.lang.get(20, self.lang.get(46)), 1,
                                                                             COLOR_RESULT_LINE), (20, 20 + 29 * k))
                                else:
                                    self.screen.blit(
                                        self.lapTimeFont.render(self.lang.get(20, self.map.getPlayer().getLapTime(obj)),
                            ...
                            if k == 2:
                                if self.map.getPlayer().getFastLap()[1] < obj:
                                    self.screen.blit(self.lapTimeFont.render(self.lang.get(21, self.lang.get(46)), 1,
                                                                             COLOR_RESULT_LINE), (20, 20 + 29 * k))
                                else:
                                    self.screen.blit(
                                        self.lapTimeFont.render(self.lang.get(21, self.map.getPlayer().getLapTime(obj)),
                                                                1, COLOR_BRONCE), (20, 20 + 29 * k))
                                    if self.map.getPlayer().getLapTimeNoFormat() > obj:
                                        pygame.draw.line(self.screen, COLOR_ALERT, (17, 31 + 29 * k),
                                                         (155, 31 + 29 * k), 3)
                            k += 1
                    ...
                    if message == STATE_OFFROAD:
                        self.screen.blit(self.OutOfRoadFont.render(self.lang.get(13), 1, COLOR_ALERT),
                                         self.posOutOfRoad)
                        red = True
                    elif message == STATE_INVALIDPOS:
                        self.screen.blit(self.OutOfRoadFont.render(self.lang.get(14), 1, COLOR_ALERT),
                                         (self.posOutOfRoad[0] - 67, self.posOutOfRoad[1]))
                        red = True
                    elif message == STATE_WRONGWAY:
                        self.screen.blit(self.OutOfRoadFont.render(self.lang.get(15), 1, COLOR_ALERT),
            ...
            # Si ya termino la pista se muestran los resultados
                ...
                self.screen.blit(self.resultsFontTitle.render(self.lang.get(16), 1, COLOR_VELOCIMETER),
                                 (self.resultsScreenPos[0] + 10, self.resultsScreenPos[1] + 17))
                self.screen.blit(self.resultsFontTitleTrack.render(self.lang.get(17) + self.map.getTrackTitle(), 1,
                                                                   COLOR_VELOCIMETER),
                                 (self.resultsScreenPos[0] + 223, self.resultsScreenPos[1] + 35))
                self.screen.blit(self.resultsFontTitleSubtl.render(self.lang.get(18), 1, COLOR_VELOCIMETER),
                                 (self.resultsScreenPos[0] + 49, self.resultsScreenPos[1] + 70))
                pygame.draw.line(self.screen, COLOR_RESULT_LINE,
                                 (self.resultsScreenPos[0] + 191, self.resultsScreenPos[1] + 62),
                                 (self.resultsScreenPos[0] + 191, self.resultsScreenPos[1] + 399))
                pygame.draw.line(self.screen, COLOR_RESULT_LINE,
                                 (self.resultsScreenPos[0] + 191, self.resultsScreenPos[1] + 62),
                                 (self.resultsScreenPos[0] + 679, self.resultsScreenPos[1] + 62))
                pygame.draw.line(self.screen, COLOR_RESULT_LINE,
                                 (self.resultsScreenPos[0] + 191, self.resultsScreenPos[1] + 62),
                                 (self.resultsScreenPos[0] + 191, self.resultsScreenPos[1] + 399))
                pygame.draw.line(self.screen, COLOR_RESULT_LINE,
                                 (self.resultsScreenPos[0] + 433, self.resultsScreenPos[1] + 62),
                                 (self.resultsScreenPos[0] + 433, self.resultsScreenPos[1] + 399))
                # Tipo de auto
                self.screen.blit(self.resultsFontTitleSubtl2.render(self.lang.get(42), 1, COLOR_VELOCIMETER),
                                 (self.resultsScreenPos[0] + 470, self.resultsScreenPos[1] + 38))
                tipo = self.map.getPlayer().getType()
                if tipo == 1:
                    self.screen.blit(self.resultsFontTitleSubtl2.render(self.lang.get(43), 1, COLOR_VELOCIMETER),
                                     (self.resultsScreenPos[0] + 590, self.resultsScreenPos[1] + 38))
                elif tipo == 2:
                    self.screen.blit(self.resultsFontTitleSubtl2.render(self.lang.get(44), 1, COLOR_VELOCIMETER),
                                     (self.resultsScreenPos[0] + 590, self.resultsScreenPos[1] + 38))
                elif tipo == 3:
                    self.screen.blit(self.resultsFontTitleSubtl2.render(self.lang.get(45), 1, COLOR_VELOCIMETER),
                                     (self.resultsScreenPos[0] + 590, self.resultsScreenPos[1] + 38))
                # Resultados
                ...
                self.screen.blit(self.resultsFontTitleSubtl2.render(self.lang.get(19, ""), 1, COLOR_ORO),
                                 (self.resultsScreenPos[0] + 10, self.resultsScreenPos[1] + 118))
                self.screen.blit(self.resultsFontTitleSubtl2.render(self.lang.get(20, ""), 1, COLOR_PLATA),
                                 (self.resultsScreenPos[0] + 10, self.resultsScreenPos[1] + 178))
                self.screen.blit(self.resultsFontTitleSubtl2.render(self.lang.get(21, ""), 1, COLOR_BRONCE),
                                 (self.resultsScreenPos[0] + 10, self.resultsScreenPos[1] + 238))
                if tiempo < oro:
                    self.screen.blit(self.resultsFontTitleSubtl2.render(self.lang.get(46), 1, COLOR_COMPLETED),
                                     (self.resultsScreenPos[0] + 50, self.resultsScreenPos[1] + 118))
                    self.screen.blit(self.resultsFontTitleSubtl2.render(
                        self.lang.get(48, self.map.getPlayer().getLapTime(oro - tiempo)), 1, COLOR_COMPLETED),
                        (self.resultsScreenPos[0] + 50, self.resultsScreenPos[1] + 138))
                else:
                    self.screen.blit(self.resultsFontTitleSubtl2.render(self.lang.get(47), 1, COLOR_ALERT),
                                     (self.resultsScreenPos[0] + 50, self.resultsScreenPos[1] + 118))
                    self.screen.blit(self.resultsFontTitleSubtl2.render(
                        self.lang.get(49, self.map.getPlayer().getLapTime(tiempo - oro)), 1, COLOR_ALERT),
                        (self.resultsScreenPos[0] + 50, self.resultsScreenPos[1] + 138))
                if tiempo < plata:
                    self.screen.blit(self.resultsFontTitleSubtl2.render(self.lang.get(46), 1, COLOR_COMPLETED),
                                     (self.resultsScreenPos[0] + 63, self.resultsScreenPos[1] + 178))
                    self.screen.blit(self.resultsFontTitleSubtl2.render(
                        self.lang.get(48, self.map.getPlayer().getLapTime(plata - tiempo)), 1, COLOR_COMPLETED),
                        (self.resultsScreenPos[0] + 50, self.resultsScreenPos[1] + 198))
                else:
                    self.screen.blit(self.resultsFontTitleSubtl2.render(self.lang.get(47), 1, COLOR_ALERT),
                                     (self.resultsScreenPos[0] + 63, self.resultsScreenPos[1] + 178))
                    self.screen.blit(self.resultsFontTitleSubtl2.render(
                        self.lang.get(49, self.map.getPlayer().getLapTime(tiempo - plata)), 1, COLOR_ALERT),
                        (self.resultsScreenPos[0] + 50, self.resultsScreenPos[1] + 198))
                if tiempo < bronce:
                    self.screen.blit(self.resultsFontTitleSubtl2.render(self.lang.get(46), 1, COLOR_COMPLETED),
                                     (self.resultsScreenPos[0] + 83, self.resultsScreenPos[1] + 238))
                    self.screen.blit(self.resultsFontTitleSubtl2.render(
                        self.lang.get(48, self.map.getPlayer().getLapTime(bronce - tiempo)), 1, COLOR_COMPLETED),
                        (self.resultsScreenPos[0] + 50, self.resultsScreenPos[1] + 258))
                else:
                    self.screen.blit(self.resultsFontTitleSubtl2.render(self.lang.get(47), 1, COLOR_ALERT),
                                     (self.resultsScreenPos[0] + 83, self.resultsScreenPos[1] + 238))
                    self.screen.blit(self.resultsFontTitleSubtl2.render(
                        self.lang.get(49, self.map.getPlayer().getLapTime(tiempo - bronce)), 1, COLOR_ALERT),
                        (self.resultsScreenPos[0] + 50, self.resultsScreenPos[1] + 258))
                # Estadisticas
                self.screen.blit(self.resultsFontTitleSubtl.render(self.lang.get(31), 1, COLOR_VELOCIMETER),
                                 (self.resultsScreenPos[0] + 247, self.resultsScreenPos[1] + 70))
                self.screen.blit(self.resultsFontTitleSubtl.render(self.lang.get(28), 1, COLOR_VELOCIMETER),
                                 (self.resultsScreenPos[0] + 492, self.resultsScreenPos[1] + 70))
                self.screen.blit(self.resultsFontTitleContent.render(
                    self.lang.get(22, self.map.getPlayer().getLapTime(sum(self.map.getPlayer().getLapsTime()))), 1, \
                    COLOR_VELOCIMETER), (
                    self.resultsScreenPos[0] + 205, self.resultsScreenPos[1] + 120))
                self.screen.blit(
                    self.resultsFontTitleContent.render(self.lang.get(23, self.map.getPlayer().getFastLap()[0]), 1,
                                                        COLOR_VELOCIMETER), (
                        self.resultsScreenPos[0] + 205, self.resultsScreenPos[1] + 160))
                self.screen.blit(self.resultsFontTitleContent.render(
                    self.lang.get(24, self.map.getPlayer().getLapTime(self.map.getPlayer().getFastLap()[1])), 1, \
                    COLOR_VELOCIMETER), (
                    self.resultsScreenPos[0] + 205, self.resultsScreenPos[1] + 180))
                self.screen.blit(
                    self.resultsFontTitleContent.render(self.lang.get(25, self.map.getPlayer().getFastVel()), 1,
                                                        COLOR_VELOCIMETER), (
                ...
                self.screen.blit(self.resultsFontTitleContent.render(
                    self.map.getPlayer().getLapTime(self.map.getPlayer().getTiempoFuera()) + "s", 1, \
                    COLOR_VELOCIMETER), (
                    self.resultsScreenPos[0] + 205, self.resultsScreenPos[1] + 280))
                self.screen.blit(
                    self.resultsFontTitleContent.render(self.lang.get(27, self.map.getPlayer().getPuntaje()), 1,
                                                        COLOR_VELOCIMETER), (
                ...
                if len(scores) > 0:
                    if scores[0] == NO_ERROR:
                        self.screen.blit(self.resultsFontTitleSubtl2.render(self.lang.get(32), 1, COLOR_VELOCIMETER),
                                         (self.resultsScreenPos[0] + 453, self.resultsScreenPos[1] + 118))
                        i = 0
                        for score in scores:
                            if i == 0:
                                pass
                    ...
                    # Se imprimen mensajes de error
                        ...
                        self.screen.blit(self.resultsFontTitleContent.render(self.lang.get(35), 1, COLOR_VELOCIMETER),
                                         (self.resultsScreenPos[0] + 451, self.resultsScreenPos[1] + 120))
                    elif scores[0] == ERROR_SCOREBOARD_NOCONECTIONDB:
                        self.screen.blit(self.resultsFontTitleContent.render(self.lang.get(36), 1, COLOR_VELOCIMETER),
                                         (self.resultsScreenPos[0] + 478, self.resultsScreenPos[1] + 120))
                    elif scores[0] == ERROR_SCOREBOARD_FAKEHASH:
                        self.screen.blit(self.resultsFontTitleContent.render(self.lang.get(37), 1, COLOR_VELOCIMETER),
                                         (self.resultsScreenPos[0] + 443, self.resultsScreenPos[1] + 120))
                        self.screen.blit(self.resultsFontTitleContent.render(self.lang.get(38), 1, COLOR_VELOCIMETER),
                                         (self.resultsScreenPos[0] + 509, self.resultsScreenPos[1] + 140))
                    elif scores[0] == ERROR_SCOREBOARD_NO_SCORES:
                        self.screen.blit(self.resultsFontTitleContent.render(self.lang.get(34), 1, COLOR_VELOCIMETER),
                                         (self.resultsScreenPos[0] + 481, self.resultsScreenPos[1] + 120))
                    elif scores[0] == ERROR_SCOREBOARD_BADPARAMETERS:
                        self.screen.blit(self.resultsFontTitleContent.render(self.lang.get(39), 1, COLOR_VELOCIMETER),
                                         (self.resultsScreenPos[0] + 478, self.resultsScreenPos[1] + 120))
                    elif scores[0] == ERROR_SCOREBOARD_FAKESCORE:
                        self.screen.blit(self.resultsFontTitleContent.render(self.lang.get(40), 1, COLOR_VELOCIMETER),
                                         (self.resultsScreenPos[0] + 480, self.resultsScreenPos[1] + 120))
                    elif scores[0] == ERROR_SCOREBOARD_FAKETIME:
                        self.screen.blit(self.resultsFontTitleContent.render(self.lang.get(41), 1, COLOR_VELOCIMETER),
                                         (self.resultsScreenPos[0] + 453, self.resultsScreenPos[1] + 120))
                    elif scores[0] == ERROR_SCOREBOARD_FAKETRACK:
                        self.screen.blit(self.resultsFontTitleContent.render(self.lang.get(50), 1, COLOR_VELOCIMETER),
                                         (self.resultsScreenPos[0] + 451, self.resultsScreenPos[1] + 120))
                else:
                    self.screen.blit(self.resultsFontTitleContent.render(self.lang.get(34), 1, COLOR_VELOCIMETER),
                                     (self.resultsScreenPos[0] + 481, self.resultsScreenPos[1] + 120))
                ...
                if not self.map.resultsSaved():
                    ...
                    save = open(
                        DIR_SAVES + str(hash(self.map.getTrackTitle())) + str(hash(self.map.getPlayer().username)) + \
                ...
                if self.map.getPlayer().getNextTrack() != TRACK_NOT_DEFINED:
                    ...
                    next_text = self.buttonsResultsFont.render(self.lang.get(109, self.map.getPlayer().getNextTrack()),
                                                               1, COLOR_VELOCIMETER)
                    next_text_width = next_text.get_size()[0]
                    pygame.gfxdraw.filled_polygon(self.screen, [
                        (self.resultsButtonQuitPos[0] - 15, self.resultsButtonQuitPos[1] - 8),
                        (self.resultsButtonQuitPos[0] + quit_text_width + 15, self.resultsButtonQuitPos[1] - 8), \
                        (self.resultsButtonQuitPos[0] + quit_text_width + 15, self.resultsButtonQuitPos[1] + 30),
                        (self.resultsButtonQuitPos[0] - 15, self.resultsButtonQuitPos[1] + 30)], \
                                                  (0, 0, 0, 230))
                    pygame.gfxdraw.filled_polygon(self.screen, [
                        (self.resultsButtonNextPos[0] - 15 - next_text_width, self.resultsButtonNextPos[1] - 8),
                        (self.resultsButtonNextPos[0] + 14, self.resultsButtonNextPos[1] - 8), \
                        (self.resultsButtonNextPos[0] + 14, self.resultsButtonNextPos[1] + 30),
                        (self.resultsButtonNextPos[0] - 15 - next_text_width, self.resultsButtonNextPos[1] + 30)], \
                                                  (0, 0, 0, 230))
                    self.screen.blit(quit_text, (self.resultsButtonQuitPos[0], self.resultsButtonQuitPos[1]))
                    self.screen.blit(next_text,
                                     (self.resultsButtonNextPos[0] - next_text_width, self.resultsButtonNextPos[1]))
